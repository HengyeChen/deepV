...
def filter_y(file_path):
  ...
  #  df['detected_point_absolute'] = df['detected_point_absolute'].replace('no_detected_point', 'None')
...
def detecte_point_expend(filtered_df, bed_file_path, output_csv_path="detecte_point_expend.csv"):
    ...
    for idx, row in filtered_df.iterrows():
        ...
        local_x_start = x_abs - 10
        local_x_end = x_abs + 10
...
def generate_fixed_kernel(kernel_dir, name, y_values, final_shape=(20, 100)):
    results = {}
    for y in y_values:
...
def generate_conv_output(model, name, images, y_values, kernel_dir='kernel_generate/kernel'):
    conv_layer = model.get_layer('conv2')
    fixed_kernel, num_kernels = generate_fixed_kernel(kernel_dir=kernel_dir, name=name, y_values=y_values)
...
def predict_and_process_batches(model, name, images, y_values, channel_name_y_values, base_dir,
                                kernel_dir='kernel_generate/kernel', kernel_height=20, kernel_width=100,
                                stride_height=1, stride_width=1, batch_size=None):
    conv_layer = model.get_layer('conv2')
    fixed_kernel, num_kernels = generate_fixed_kernel(kernel_dir=kernel_dir, name=name, y_values=y_values)
    fixed_bias = np.zeros(num_kernels)
    conv_layer.set_weights([fixed_kernel, fixed_bias])
...
print('count_y_values', count_y_values)
