...
def collect_rows(max_csv_path: str, min_csv_path: str, detect_type: str,
                 min_relative_height: float = 0.3, sensitivity: float = 0.5,
                 window_size: int = 5, verbose: bool = False) -> List[Tuple[str, Any, Any, Any, Any]]:
    ...
    if detect_type in ['max', 'both'] and max_csv_path:
        ...
        for i in max_result['max_indices']:
            rows.append(("max", max_result['x'][i], max_result['y'][i], "", ""))
    ...
    if detect_type in ['min', 'both'] and min_csv_path:
        ...
        for info in min_result['minima_info']:
            x_left = info['x_left'] or ""
            x_right = info['x_right'] or ""
            rows.append(("min", info['x_lowest'], min_result['y'][info['index']], x_left, x_right))

    return rows

def process_files(max_csv_path: str, min_csv_path: str, detect_type: str,
                  output_file: str, min_relative_height: float = 0.3,
                  sensitivity: float = 0.5, window_size: int = 5,
                  target_image: int = None) -> None:
    if target_image is not None:
        base_name = os.path.basename(output_file)
        match = re.search(r"image_(\d+)", base_name)
        if not match or int(match.group(1)) != target_image:
            return
    rows = collect_rows(
        max_csv_path,
        min_csv_path,
    ...
    with open(output_file, 'w') as f:
        ...
        for row in rows:
            ...
            f.write(f"{row_type}\t{x_val}\t{y_val}\t{x_left}\t{x_right}\n")
...
def main():
    ...
    parser.add_argument('--target_image', type=int, default=None, help='仅处理指定image编号')
